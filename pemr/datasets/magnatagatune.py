...
from pemr.datasets import Dataset
...
def get_file_list(root, subset, split, label_factor):
    if subset == "train":
        if split == "pons2017":
            if label_factor == 0.01:
                fl = open(os.path.join(root, "train_gt_mtt_1p.tsv")).read().splitlines()
            elif label_factor == 0.1:
                fl = open(os.path.join(root, "train_gt_mtt_10p.tsv")).read().splitlines()
            elif label_factor == 0.5:
                fl = open(os.path.join(root, "train_gt_mtt_50p.tsv")).read().splitlines()
            elif label_factor == 1.0:
...
class MAGNATAGATUNE(Dataset):
    ...
    def __init__(
        ...
        self.fl, self.binary = get_file_list(self._path, self.subset, self.split, self.label_factor)
        self.n_classes = 50
